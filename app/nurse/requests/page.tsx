import { RequestLeaveForm } from "@/components/shared/request-leave-form";
import { getMyLeaveRequests } from "@/lib/actions/user.actions";
import { notFound } from "next/navigation";

function AllLeaveRequests({ requests }: { requests: any[] }) {
  return (
        ...
        <ul>
          {requests.map((request) => (
            <li key={request.id} className="mb-2 p-2 rounded-lg bg-white/10">
              ...
              {request.reason && <p className="text-sm mt-1"><strong>Reason:</strong> {request.reason}</p>}
            </li>
          ))}
        </ul>
      ) : (
...
export default async function RequestsPage() {
  ...
  return (
    <div>
      ...
      <p className="text-lg mb-8">Submit a new leave request or view your existing ones.</p>
